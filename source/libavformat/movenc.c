...
static const size_t AES_CBC_KEY_SIZE = 16;
...
static const AVOption options[] = {
    ...
    { "ism_offset", "Offset to the ISM fragment start times", offsetof(MOVMuxContext, ism_offset), AV_OPT_TYPE_INT64, {.i64 = 0}, -1, INT64_MAX, AV_OPT_FLAG_ENCODING_PARAM, "ism_offset"},
    { "epoch_time", NULL, 0, AV_OPT_TYPE_CONST, {.i64 = -1}, 0, 0, AV_OPT_FLAG_ENCODING_PARAM, "ism_offset"},
    ...
    { "encryption_scheme",    "Configures the encryption scheme, allowed values are none, cenc-aes-ctr, irdeto-cenc-aes-ctr-v3, irdeto-cbcs-aes-cbc",
                               offsetof(MOVMuxContext, encryption_scheme_str),   AV_OPT_TYPE_STRING, {.str = NULL}, .flags = AV_OPT_FLAG_ENCODING_PARAM },
...
static int mov_write_audio_tag(AVFormatContext *s, AVIOContext *pb, MOVMuxContext *mov, MOVTrack *track)
{
    ...
    switch (mov->encryption_scheme) {
            case MOV_ENC_CENC_AES_CTR:
            {
                ff_mov_cenc_write_sinf_tag(track, pb, mov->encryption_kid);
                break;
            }
            case MOV_ENC_CENC_V3_AES_CTR:
            {
                ff_mov_cencv3_write_sinf_tag(track, pb, mov->encryption_kid);
                break;
            }
            case MOV_ENC_CBCS_AES_CBC:
            {
                ff_mov_cbcs_write_sinf_tag(track, pb, mov->encryption_kid);
                break;
            }
            default:
                break;
...
static int mov_write_stbl_tag(AVFormatContext *s, AVIOContext *pb, MOVMuxContext *mov, MOVTrack *track)
{
    ...
    switch (mov->encryption_scheme) {
        case MOV_ENC_CENC_AES_CTR:
            ...
            break;
        case MOV_ENC_CENC_V3_AES_CTR:
            ff_mov_cencv3_write_stbl_atoms(&track->cenc, pb);
            break;
        case MOV_ENC_CBCS_AES_CBC:
            ff_mov_cbcs_write_stbl_atoms(&track->cbcs, pb);
            break;
        default:
            break;
    }
...
int ff_mov_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (mov->ism_offset < 0)
    {
        long long epoch_time = -1LL;
        av_wm_info_get_epoch_time(&epoch_time);

        if (epoch_time < 0LL)
        {
            // There is no real solution for this case, it is just workaround
            if (par->codec_type == AVMEDIA_TYPE_VIDEO)
            {
                av_log(s, AV_LOG_WARNING, "Initial epoch time is not specified, ISM offset will be set to 0 !!!\n");
                mov->ism_offset = 0;
            }
            else
            {
                av_log(s, AV_LOG_WARNING, "Initial epoch time is not specified, data packet (%" PRId64 " samples) will be skipped !!!\n", pkt->duration);
                trk->frag_start += pkt->duration;
                return 0;
            }
        }
        else
        {
            av_log(s, AV_LOG_INFO, "Initial epoch time: %lld\n", epoch_time);

            int i; for (i = 0; i < mov->nb_streams; i ++)
            {
                AVCodecContext*    codec        = mov->tracks[i].st->codec;
                AVCodecParameters* codec_params = mov->tracks[i].par;

                int time_base_num = codec->time_base.num;
                int time_base_den = codec->time_base.den;

                if (time_base_num > 0 && time_base_den > 0)
                {
                    uint64_t ism_offset = epoch_time * time_base_den / time_base_num;
                    mov->tracks[i].frag_start += ism_offset;

                    switch (codec_params->codec_type)
                    {
                        case AVMEDIA_TYPE_VIDEO:
                            av_log(s, AV_LOG_INFO, "Stream %d (video): ism_offset = %" PRId64 "\n", i, ism_offset);
                            break;

                        case AVMEDIA_TYPE_AUDIO:
                            av_log(s, AV_LOG_INFO, "Stream %d (audio): ism_offset = %" PRId64 "\n", i, ism_offset);
                            break;

                        default:
                            av_log(s, AV_LOG_INFO, "Stream %d: ism_offset = %" PRId64 "\n", i, ism_offset);
                            break;
                    }
                }
            }

            mov->ism_offset = epoch_time;
        }
    }
    ...
    if (par->codec_id == AV_CODEC_ID_H264 && trk->vos_len > 0 && *(uint8_t *)trk->vos_data != 1 && !TAG_IS_AVCI(trk->tag)) {
        ...
        if (trk->hint_track >= 0 && trk->hint_track < mov->nb_streams) {
            ...
            switch (mov->encryption_scheme) {
                ...
                case MOV_ENC_CENC_V3_AES_CTR:
                {
                    size = ff_mov_cencv3_avc_parse_nal_units(&trk->cenc, pb, pkt->data, size);
                    if (size < 0) {
                        ret = size;
                        goto err;
                    }
                ...
                case MOV_ENC_CBCS_AES_CBC:
                {
                    size = ff_mov_cbcs_avc_parse_nal_units(&trk->cbcs, pb, pkt->data, size);
                    if (size < 0) {
                        ret = size;
                        goto err;
                    }
        ...
        switch (mov->encryption_scheme) {
            ...
            case MOV_ENC_CENC_V3_AES_CTR:
            {
                if (par->codec_id == AV_CODEC_ID_H264 && par->extradata_size > 4) {
                    int nal_size_length = (par->extradata[4] & 0x3) + 1;
                    ret = ff_mov_cencv3_avc_write_nal_units(s, &trk->cenc, nal_size_length, pb, pkt->data, size);
                } else {
                    ret = ff_mov_cencv3_write_packet(&trk->cenc, pb, pkt->data, size);
                }
            ...
            case MOV_ENC_CBCS_AES_CBC:
            {
                if (par->codec_id == AV_CODEC_ID_H264 && par->extradata_size > 4) {
                    int nal_size_length = (par->extradata[4] & 0x3) + 1;
                    ret = ff_mov_cbcs_avc_write_nal_units(s, &trk->cbcs, nal_size_length, pb, pkt->data, size);
                } else {
                    ret = ff_mov_cbcs_write_packet(&trk->cbcs, pb, pkt->data, size);
                }
    ...
    for (i = 0; i < mov->nb_streams; i++) {
        ...
        if (mov->tracks[i].vos_len)
            ...
            switch (mov->encryption_scheme) {
                ...
                case MOV_ENC_CENC_V3_AES_CTR:
                {
                    ff_mov_cencv3_free(&mov->tracks[i].cenc);
                    break;
                }
                case MOV_ENC_CBCS_AES_CBC:
                {
                    ff_mov_cbcs_free(&mov->tracks[i].cbcs);
                    break;
                }
                default:
                    break;
...
static int mov_init(AVFormatContext *s)
{
    ...
    if (mov->encryption_scheme_str != NULL && strcmp(mov->encryption_scheme_str, "none") != 0) {
        ...
        else if (strcmp(mov->encryption_scheme_str, "irdeto-cenc-aes-ctr-v3") == 0) {
            mov->encryption_scheme = MOV_ENC_CENC_V3_AES_CTR;
        ...
        else if (strcmp(mov->encryption_scheme_str, "irdeto-cbcs-aes-cbc") == 0) {
            mov->encryption_scheme = MOV_ENC_CBCS_AES_CBC;

            if (mov->encryption_key_len != AES_CBC_KEY_SIZE) {
                av_log(s, AV_LOG_ERROR, "Invalid encryption key len %d expected %d\n",
                    mov->encryption_key_len, AES_CBC_KEY_SIZE);
                return AVERROR(EINVAL);
            }

            if (mov->encryption_kid_len != CBCS_KID_SIZE) {
                av_log(s, AV_LOG_ERROR, "Invalid encryption kid len %d expected %d\n",
                    mov->encryption_kid_len, CBCS_KID_SIZE);
                return AVERROR(EINVAL);
            }
        } else {
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        /* The Protected Interoperable File Format (PIFF) standard, used by ISMV recommends but
           ...
           for video and audio tracks, so if user-set, it isn't overwritten */
        if (mov->mode == MODE_ISM &&
        ...
        switch (mov->encryption_scheme) {
            ...
            case MOV_ENC_CENC_V3_AES_CTR:
            {
                ret = ff_mov_cencv3_init(&track->cenc, mov->encryption_key,
                    track->par->codec_id == AV_CODEC_ID_H264, s->flags & AVFMT_FLAG_BITEXACT);
                if (ret)
                    return ret;
                ...
                if (track->par->codec_id == AV_CODEC_ID_H264)
                {
                    //parse xps from extra data prepared for vcl nal slice header parsing
                    ret = ff_mov_cencv3_parse_avc_XPS(&track->cenc, track->par->extradata, track->par->extradata_size);
                    if (ret) return ret;
                }
                break;
            }
            case MOV_ENC_CBCS_AES_CBC:
            {
                ff_mov_cbcs_set_random_iv(mov->encryption_iv, s->flags & AVFMT_FLAG_BITEXACT);
                ret = ff_mov_cbcs_init(&track->cbcs, mov->encryption_key, mov->encryption_iv,
                                       CBCS_PATTERN_DEFAULT_CRYPT_BLOCK_NUM,
                                       CBCS_PATTERN_DEFAULT_SKIP_BLOCK_NUM,
                                       track->par->codec_id == AV_CODEC_ID_H264);
                if (ret)
                    return ret;
                ...
                if (track->par->codec_id == AV_CODEC_ID_H264)
                {
                    ret = ff_mov_avc_cbcs_parse_XPS(&track->cbcs, track->par->extradata, track->par->extradata_size);
                    if (ret) return ret;
                }
                break;
            }
