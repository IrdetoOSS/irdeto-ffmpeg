...
typedef struct MOVStreamContext {
    ...
    /**
    * @note Irdeto adds cbcs decryption patch which reuses the cenc struct as most members are common.
    */
    struct {
        struct AVAESCTR *aes_ctr;       //< for cenc scheme
        struct AVAES *aes_cbc;  //for cbcs scheme
...
typedef struct MOVContext {
    ...
    int enable_irdeto_cbcs;     //< switch on/off irdeto patch about CENC cbcs decryption.
} MOVContext;
...
int ff_mov_read_stsd_entries(MOVContext * c, AVIOContext * pb,
                             int entries);
...
static inline enum AVCodecID ff_mov_get_lpcm_codec_id(int bps, int flags)
{
     ...
     * 0x4 = signed
     */
    return ff_get_pcm_codec_id(bps, flags & 1, flags & 2,
                               flags & 4 ? -1 : 0);
}
