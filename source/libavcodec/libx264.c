                       ...
                       const x264_nal_t *nals, int nnal)
{
    ...
    {
        /**
        * @note skip non pps or vcl for irdeto patch
        */
        if (x4->enable_irdeto_exports && !(is_x264_pps(nals[i].i_type) || is_x264_vcl(nals[i].i_type)))
            continue;
...
static void adjust_params_for_watermarking(
    ...
    param->b_cabac = x4->params.b_cabac;
    ...
    param->i_log_level = X264_LOG_DEBUG; //X264_LOG_NONE;
    param->pf_log = X264_log; //log_callback;
    param->b_repeat_headers = 0;

    param->rc.i_qp_constant = ctx->global_quality;
    param->rc.i_qp_min = ctx->global_quality;
    param->rc.i_qp_max = ctx->global_quality;
    param->pps_id = x4->irdeto_pps_id;

    param->i_width = ctx->width;
    param->i_height = ctx->height;
    param->i_csp = convert_pix_fmt(ctx->pix_fmt);

    param->b_irdeto_extension = 1;
}

static av_cold int X264_close(AVCodecContext *ctx);
                      ...
                      int *got_packet)
{
    ...
    if(x4->enable_irdeto_exports && frame) {
        if (x4->enc) {
        ...
        if (!x4->enc) {
            av_log(ctx, AV_LOG_ERROR, "Cannot re-open libx264 encoder for watermarking.\n");
            X264_close(ctx);
    ...
    if (*got_packet && x4->enable_irdeto_exports) {
        ...
        for (int i = 0; i < nnal; i++) {
            if (nal[i].i_type == NAL_PPS) {
                uint8_t *pps_ptr = av_malloc(nal[i].i_payload);
                memcpy(pps_ptr, nal[i].p_payload, nal[i].i_payload);
                av_packet_add_side_data(pkt, AV_PKT_IRDETO_PPS, pps_ptr, nal[i].i_payload);
            }
        }
...
static av_cold int X264_init(AVCodecContext *avctx)
{
    ...
    if (avctx->global_quality > 0 && !x4->enable_irdeto_exports)
    ...
    if (avctx->refs >= 0)
        ...
        for (i = 0; i < x264_levels[i].level_idc; i++)
            if (x264_levels[i].level_idc == level_id)
                x4->params.i_frame_reference = av_clip(x264_levels[i].dpb / mbn / scale, 1,
    ...
    x4->params.i_slice_count = avctx->slices; //!mv
