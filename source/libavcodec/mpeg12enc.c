                                                      ...
                                                      int mb_block_count)
{
    ...
    if (cbp == 0 && !first_mb && s->mv_type == MV_TYPE_16X16 &&
        ...
        if (first_mb) {
        	...
        	if (s->enable_irdeto_exports && s->irdeto_imx && s->pict_type == AV_PICTURE_TYPE_I) {
        		/* In case of IMX, write slice header for every mb */
        		ff_mpeg1_encode_slice_header(s);
        ...
        if (s->pict_type == AV_PICTURE_TYPE_I) {
            if (s->dquant && cbp) {
            	if (s->enable_irdeto_exports && s->irdeto_imx && !first_mb)
            	{
            		/* Skip qscale as qscale already set in slice header in case of IMX */
